...
// --- Конструкторы ---
    SimpleForecast::SimpleForecast()
            : timestamp(std::chrono::system_clock::now()), morningTemp(0), dayTemp(0), eveningTemp(0),
              weather(Weather::Sunny), precipitation(0) {}

    SimpleForecast::SimpleForecast(std::time_t ts, float morning, float day, float evening, Weather w, float precip)
            : timestamp(std::chrono::system_clock::from_time_t(ts)), morningTemp(morning), dayTemp(day),
              eveningTemp(evening), weather(w), precipitation(precip) {}

    SimpleForecast::SimpleForecast(std::time_t ts, float morning, float day, float evening, float precip)
            : timestamp(std::chrono::system_clock::from_time_t(ts)), morningTemp(morning), dayTemp(day),
              eveningTemp(evening), precipitation(precip) {
...
// --- Проверка, является ли прогноз ошибочным ---
    bool SimpleForecast::isInvalidForecast() const {
        if (morningTemp < -100 || morningTemp > 60 || dayTemp < -100 || dayTemp > 60 || eveningTemp < -100 ||
            eveningTemp > 60) {
            return true;
        }
...
// --- Оператор += для объединения прогнозов ---
    SimpleForecast &SimpleForecast::operator+=(const SimpleForecast &other) {
        if (this->timestamp != other.timestamp) {
            throw std::invalid_argument("The forecasts must be for the same date!");
        }
    ...
    std::istream &operator>>(std::istream &in, SimpleForecast &forecast) {
        ...
        switch (weather_choice) {
            case 1:
                ...
                break;
            case 2:
                forecast.setWeather(Weather::Cloudy);
                break;
            case 3:
                forecast.setWeather(Weather::Rain);
                break;
            case 4:
                forecast.setWeather(Weather::Snow);
                break;
            case 5:
                forecast.setWeather(Weather::Windy);
                break;
            case 6:
                forecast.setWeather(Weather::Fog);
                break;
            default:
                forecast.setWeather(Weather::Sunny);
        }
